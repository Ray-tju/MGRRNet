...
class OccupancyNetwork(nn.Module):
    ...
    def forward(self, p, inputs, x1, x2, sample=True, train_step=0, **kwargs):
        ...
        '''
        batch_size = p.size(0)
        xfg1, xfg2, x0, concate_out = self.encode_inputs(inputs, x1, x2)
        z = self.get_z_from_prior((batch_size,), sample=sample)
        out_concat, out_xfg1, out_xfg2, out = self.decode(p, z, z, z, z, xfg1, xfg2, x0, concate_out, train_step,
                                                          **kwargs)
        return out_concat, out

    def compute_elbo(self, p, occ, inputs, x1, x2, train_step=0, **kwargs):
        ...
        rec_error = -out_concat.log_prob(occ).sum(dim=-1)
        kl = dist.kl_divergence(q_z, self.p0_z).sum(dim=-1)
        elbo = -rec_error - kl

        return elbo, rec_error, kl

    def encode_inputs(self, inputs, x1, x2):
        ''' Encodes the input.
    ...
    def decode(self, p, z_xfg1, z_xfg2, z_x0, z_concate_out, xfg1, xfg2, x0, concate_out, train_step, **kwargs):
        ...
        out_concat, out_xfg1, out_xfg2, out = self.decoder(p, z_xfg1, z_xfg2, z_x0, z_concate_out, xfg1, xfg2, x0,
                                                           concate_out, train_step, **kwargs)
        ...
        return p_r_out_concat, p_r_out_xfg1, p_r_out_xfg2, p_r_out
